...
def recommend_code_first(query: str, top_k: int = TOP_K):
    ...
    def code_score(row):
        ...
        score += 3.0 * lexical_similarity(q, code)
    ...
    for _, row in candidates.iterrows():
        normalized = min(float(row["code_score"]) / 10.0, 1.0)
        results.append(make_result(row, normalized))
    return results
...
def evaluate_method(method_name, eval_df):
    ...
    for _, row in eval_df.iterrows():
        ...
        elif method_name == "tfidf_reranked":

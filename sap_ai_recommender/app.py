...
# ARTIFACTS CONFIG
# ====================================================
ARTIFACTS_DIR = Path(os.environ.get("ARTIFACTS_DIR", "output"))
...
DATA_PATH_GZ = ARTIFACTS_DIR / "transactions_model.csv.gz"
DATA_PATH_CSV = ARTIFACTS_DIR / "transactions_model.csv"

if DATA_PATH_GZ.exists():
    DATA_PATH = DATA_PATH_GZ
...
def ensure_artifacts():
    ...
    if missing_files:
        raise RuntimeError(
            ...
            + "\n".join(missing_files)
            + "\n\nExpected files:\n"
            "output/vectorizer.joblib\n"
            "output/matrix.npz\n"
            "output/transactions_model.csv.gz OR output/transactions_model.csv\n\n"
            "For local test with current /tmp artifacts, run:\n"
            "ARTIFACTS_DIR=/tmp/sap_ai_recommender_artifacts python app.py"
        )

    if empty_files:
        raise RuntimeError(
            "❌ Empty/corrupted artifacts detected:\n"
            + "\n".join(empty_files)
            + "\n\nDelete these empty files and use valid artifacts."
        )

    print(f"✅ Artifacts found in {ARTIFACTS_DIR}")
...
try:
    ...
    raise RuntimeError(
        f"❌ Failed to load vectorizer: {VECTORIZER_PATH}\n"
        f"Reason: {repr(e)}"
    )
...
try:
    ...
    raise RuntimeError(
        f"❌ Failed to load matrix: {MATRIX_PATH}\n"
        f"Reason: {repr(e)}"
    )

print("Building NearestNeighbors model in memory...")
model = NearestNeighbors(
    metric="cosine",
    algorithm="brute",
    n_neighbors=min(25, X.shape[0])
)
model.fit(X)
...
try:
    ...
    raise RuntimeError(
        f"❌ Failed to load transactions data: {DATA_PATH}\n"
        f"Reason: {repr(e)}"
    )
...
if missing_columns:
    raise RuntimeError(f"❌ Missing required columns in transactions data: {missing_columns}")
...
def is_prefix_code_query(query: str) -> bool:
    ...
    return bool(re.fullmatch(r"[/A-Z0-9_]+", q)) and 2 <= len(q) <= 12
...
def detect_cluster(row):
    ...
    if tcode.startswith("HR") or any(
        k in desc
        for k in ["PAYROLL", "PERSONNEL", "HUMAN RESOURCES", "EMPLOYEE"]
    ):
        return "HR / HCM"
...
def rank_alias_match(exact_code: str, query: str = "", top_k: int = TOP_K):
    ...
    distances, indices = model.kneighbors(
        X[idx],
        n_neighbors=min(top_k * 5, len(df))
    )
...
def rank_semantic_text(query: str, top_k: int = TOP_K):
    ...
    distances, indices = model.kneighbors(
        query_vec,
        n_neighbors=min(top_k * 7, len(df))
    )
...
def recommend_lightweight_hybrid(raw_query: str, top_k: int = TOP_K):
    ...
    tfidf_map = {
        str(item["tcode"]).upper(): float(item["score"])
...
def recommend_tfidf_reranked(raw_query: str, top_k: int = TOP_K):
    ...
    tfidf_rank_map = {
        str(item["tcode"]).upper(): rank
        for rank, item in enumerate(tfidf_candidates, start=1)
    }
...
def recommend_api():
    ...
    mode = data.get("mode", "hybrid_engine")

    print(f"[RECOMMEND] query={query!r} mode={mode!r}")

    if not str(query).strip():
        return jsonify({"query": query, "mode": mode, "results": []})
    ...
    try:
        ...
        for i, item in enumerate(results, start=1):
            row = {
                ...
                # Compatibility with frontend table fields
        ...
        print(f"❌ Recommendation error: {repr(e)}")
        return jsonify({
            "query": query,
            "mode": mode,
            "error": str(e),
